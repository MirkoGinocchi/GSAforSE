...
RXB_samples = X(:,1:3*P.Nbranches);
Z_real = X(:,3*P.Nbranches + 1:end);
...
parfor MCit = 1:length(X(:,1))
    ...
    zdata = zdatas(Z_real(MCit,:), P.sigma_meas, P.nvi, P.npi, P.nqi, [], [], P.npf, P.nqf,...
                            P.V_meas_bus, P.P_meas_bus, [], P.PQf_meas_branch, [], bus_indices_branch); % Get Measurement data
...
%        h3 = zeros(nqi,1);
        ...
        H = [H11 H12; H21 H22; H41 H42; H51 H52];
        H_tilde = W_onehalf * H;
        r_tilde = W_onehalf * r;
        [Q_n,U] = qr(H_tilde,0);
        r_q = Q_n' * r_tilde;
